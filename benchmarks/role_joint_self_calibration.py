...
from benchmarks.role_joint_boundary_trajectory_edges import (
    edge_directional_score,
    build_edge_router,
)
...
def representative_anchor_tuples(spec):
    """Create bounded deterministic positive examples only from registered anchors.

    For each valid role pattern, rotate through anchor indices so calibration does not
    depend on one privileged synonym. No novel/context-only test token is used here.
    """
    roles = spec["roles"]
    ...
    for pattern in patterns:
        for shift in range(max_anchors):
            tokens = tuple(
                roles[role][shift % len(roles[role])]
                for role in pattern
            )
...
def calibration_rows(router, spec):
    ...
    for positive_tokens, _pattern in representative_anchor_tuples(spec):
        # Exhaustive permutations are bounded here by the experimental sequence
        # lengths (4 or 6). This is calibration, not query-time search.
    ...
    # Deduplicate identical token permutations generated by multiple positives.
    dedup = {}
    for row in rows:
...
def main():
    ...
    print(json.dumps({
        "method": "per-router deterministic self-calibration from registered role anchors and structural counterfactual permutations",
