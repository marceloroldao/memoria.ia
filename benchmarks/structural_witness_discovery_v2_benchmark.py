...
def run(size: int) -> dict[str, object]:
    ...
    # Dense structural bridge: many distinct sources share the same bridge+hub.
    # The detector must fail closed rather than manufacture pairwise equivalence.
    dense = tuple(
        TrajectoryOccurrence(
...
def main() -> None:
    ...
    result = {
        "schema": "memoria.structural-witness-discovery.v2.benchmark.1",
        "constraints": {
