...
CONCEPTS = {
    "source.voltage": {
        "payload": "A fonte principal trabalha com 24 V.",
        "examples": [
            "a fonte principal trabalha com 24 volts",
            "a tensão da fonte é 24 v",
            "alimentação principal em vinte e quatro volts",
        ],
    },
    "source.current": {
        "payload": "A fonte principal suporta 10 A.",
        "examples": [
            "a corrente da fonte é 10 amperes",
            "a fonte suporta dez amperes",
            "corrente máxima da alimentação em 10 a",
        ],
    },
    "customer.plan": {
        "payload": "O cliente está no plano fibra 600 Mbps.",
        "examples": [
            "o cliente usa plano de 600 mega",
            "assinatura fibra de 600 mbps",
            "velocidade contratada pelo cliente é 600 mega",
        ],
    },
    "invoice.status": {
        "payload": "A fatura de agosto está paga.",
        "examples": [
            "a fatura de agosto foi paga",
            "pagamento da conta de agosto confirmado",
            "cobrança de agosto quitada",
        ],
    },
    "network.outage": {
        "payload": "Há uma indisponibilidade total da rede no prédio.",
        "examples": [
            "a internet caiu para todo o prédio",
            "ninguém no prédio consegue acessar a rede",
            "indisponibilidade total de internet no edifício",
        ],
    },
    "network.latency": {
        "payload": "A rede está conectada, porém com latência alta.",
        "examples": [
            "a internet funciona mas o ping está alto",
            "rede conectada com latência elevada",
            "conexão ativa porém muito lenta para responder",
        ],
    },
    "fiber.break": {
        "payload": "A fibra óptica foi rompida por uma escavadeira.",
        "examples": [
            "a escavadeira rompeu a fibra",
            "o cabo óptico foi cortado",
            "houve rompimento físico da fibra",
        ],
    },
    "router.failure": {
        "payload": "O roteador principal está com falha de hardware.",
        "examples": [
            "o roteador principal apresentou defeito",
            "falha de hardware no roteador",
            "equipamento de roteamento parou de funcionar",
        ],
    },
}

# Counterexamples only teach distinctions; they do not contain the expected answer.
COUNTEREXAMPLES = {
    "network.outage": [
        "a internet não caiu mas o ping está alto",
        "somente um computador está sem acesso",
    ],
    "network.latency": [
        "ninguém consegue acessar a internet porque a rede caiu",
    ],
    "fiber.break": [
        "a fibra não rompeu e o sinal óptico está normal",
    ],
    "router.failure": [
...
def main() -> None:
    ...
    for concept_id, spec in CONCEPTS.items():
        memory.remember(
            ...
            spec["payload"],
            ("key", concept_id),
            provenance="autonomous-semantic-retrieval-v1",
        )
    ...
    for query, expected, category in QUERIES:
        ...
        if predicted is not None:
            # Crucial condition: the query supplies NO memory key.  The router's
            # predicted concept becomes the trajectory used for O(1) recall.
            record = memory.recall(scope, ("key", predicted))
