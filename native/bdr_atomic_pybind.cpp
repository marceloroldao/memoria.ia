...
std::string as_bytes(py::handle value) {
    if (!PyBytes_Check(value.ptr())) throw py::type_error("value must be bytes");
    char* data = nullptr;
    Py_ssize_t size = 0;
    if (PyBytes_AsStringAndSize(value.ptr(), &data, &size) != 0) throw py::error_already_set();
...
#undef G
    ...
    void increment(std::uint64_t n) { t0 += n; if (t0 < n) ++t1; }
    void update(const unsigned char* in, std::size_t len) {
        while (len > 0) {
            ...
            buflen += take; in += take; len -= take;
            if (buflen == 128 && len > 0) {
                increment(128); compress(buf.data(), false); buflen = 0;
            }
        }
    }
...
std::uint64_t decode_u64(const std::optional<std::string>& value) {
    return value ? static_cast<std::uint64_t>(std::stoull(*value)) : 0;
}
std::string encode_u64(std::uint64_t value) { return std::to_string(value); }

bdr::DurabilityMode durability(bool durable) {
    return durable ? bdr::DurabilityMode::BatchSync : bdr::DurabilityMode::Async;
}
...
public:
    NativeDatabase(const std::string& path, std::size_t reserve_bytes, std::size_t wal_batch) {
        (void)reserve_bytes;
        (void)wal_batch;
        db_ = bdr::AtomicDatabase::open(std::filesystem::path(path));
    }

    ~NativeDatabase() {
        try { if (db_) db_->sync(); } catch (...) {}
    }
    ...
    std::uint64_t put_many(py::iterable rows, bool durable_flag) {
        ...
        for (py::handle item : rows) {
            ...
            entries.emplace_back(py::cast<std::string>(row[0]), as_bytes(row[1]));
        }
        py::gil_scoped_release release;
        return db_->put_many(std::move(entries), durability(durable_flag)).sequence;
    }

    py::dict add_resolutive_memory(const std::string& memory_id, py::bytes payload, int max_layer, bool durable_flag) {
...

        {
            ...
            for (int layer = 0; layer <= max_layer; ++layer) {
                per_layer[static_cast<std::size_t>(layer)] = decode_u64(
                    db_->get("meta:nodes_layer:" + std::to_string(layer))
                );
            }
            ...
            for (int layer = 0; layer <= max_layer; ++layer) {
                batch.emplace_back(
                    "meta:nodes_layer:" + std::to_string(layer),
                    encode_u64(per_layer[static_cast<std::size_t>(layer)])
                );
            }

            db_->put_many(std::move(batch), durability(durable_flag));
        }
    ...
    std::uint64_t erase_many(std::vector<std::string> keys, bool durable_flag) {
        py::gil_scoped_release release;
        return db_->erase_many(std::move(keys), durability(durable_flag)).sequence;
    }

    void sync() { py::gil_scoped_release release; db_->sync(); }
    void checkpoint() { py::gil_scoped_release release; db_->sync(); }
    void close() {
        if (!db_) return;
        py::gil_scoped_release release;
        db_->sync();
...
private:
    std::unique_ptr<bdr::AtomicDatabase> db_;
};

}  // namespace

PYBIND11_MODULE(_bdr_native, m) {
    m.doc() = "Direct pybind11 binding to Resolutive-DB v1.1 AtomicDatabase";
