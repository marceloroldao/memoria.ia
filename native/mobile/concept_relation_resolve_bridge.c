...
/* Keep this internal layout synchronized with memoria_mobile.c. The public ABI
 * remains opaque; this bridge is compiled as part of the same native runtime. */
...
static int bridge_build_path_context(
    const memoria_concept_relation_path *path,
    char *out,
    size_t cap
) {
    size_t i, used = 0;
...
static int bridge_build_evidence_json(
    const memoria_concept_relation_path *path,
    char *out,
    size_t cap
) {
...
static memoria_mobile_status bridge_set_hit(
    ...
    if (!bridge_build_path_context(path, context, sizeof(context)) ||
        !bridge_build_evidence_json(path, evidence, sizeof(evidence)))
    ...
    needed = snprintf(NULL, 0,
        ...
        "\"relation_inference_used\":true,\"inference_hops\":%zu,\"inference_evidence_ids\":%s}",
        path->confidence, evidence, escaped, path->hops, evidence);
...
memoria_mobile_status memoria_mobile_resolve_context_json(
    ...
    if (!namespace_id) namespace_id = bridge_json_string("{\"namespace\":\"\"}", "namespace");

    if (!source || !source[0] || !target || !target[0] ||
        !concept_namespace || !concept_namespace[0] || !handle->concept_runtime) goto done;

    relation_status = memoria_concept_relation_runtime_infer(
        handle->turns,
        handle->turn_count,
        namespace_id ? namespace_id : "",
        memoria_concept_runtime_index(handle->concept_runtime),
        concept_namespace,
        source,
        target,
        query ? query : "",
    ...
    if (relation_status == MEMORIA_CONCEPT_RELATION_RUNTIME_HIT && path_count == 1u) {
        ...
        base_status = bridge_set_hit(response_json, &paths[0]);
    }
