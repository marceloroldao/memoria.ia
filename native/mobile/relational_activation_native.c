...
/* Private ABI-v1 handle layout. Kept identical to memoria_mobile.c. */
...
static int resolve_activation_key(
    ...
    resolution = memoria_concept_resolve_with_context(
        concept_index, concept_namespace ? concept_namespace : "", surface, ""
    );
...
static memoria_mobile_status set_activation_response(
    ...
    }
    {
        ...
        snprintf(tail, sizeof(tail),
            ...
            relations_json ? relations_json : "[]", relation_count, max_depth, confidence);
...
memoria_mobile_status memoria_mobile_activate_relations_json(
    ...
    if (!namespace_id) namespace_id = strdup("");
    if (!concept_namespace) concept_namespace = strdup("");
    ...
    if (!concept || !*concept || !namespace_id || !concept_namespace || depth_raw < 1 || depth_raw > (long)ACTIVATION_MAX_DEPTH ||
        budget_raw < 64 || budget_raw > (long)(ACTIVATION_CONTEXT_CAP - 1u) || hop_decay <= 0.0 || hop_decay > 1.0 ||
    ...
    if (memoria_concept_relation_build_edges(
            h->turns, h->turn_count, namespace_id,
            memoria_concept_runtime_index(h->concept_runtime), concept_namespace,
            storage, max_edges, &edge_count) != MEMORIA_CONCEPT_RELATION_ADAPTER_OK) {
        status = set_activation_response(response_json, "UNRESOLVED", concept, "", "[]", 0u, max_depth, 0.0);
        goto done;
    }
    snprintf(frontier[0].node_key, sizeof(frontier[0].node_key), "%s", start_key);
    frontier[0].depth = 0u;
    frontier[0].confidence = 1.0;
    frontier_count = 1u;
    visited[0] = frontier[0]; visited_count = 1u;
    ...
    while (frontier_index < frontier_count) {
        ...
        for (i = 0; i < edge_count; ++i) {
            ...
            effective = current.confidence * edge->confidence * hop_decay;
            if (effective < min_confidence || !next_key || !*next_key) continue;
            snprintf(line, sizeof(line), "%s | %s | %s [hop=%u confidence=%.3f]",
                edge->subject_key, edge->predicate ? edge->predicate : "", edge->object_key,
                current.depth + 1u, effective);
            if (result_count && context_used + 1u < budget) {
                if (!append_text(context, budget + 1u, &context_used, "\n")) goto budget_done;
            }
            if (context_used + strlen(line) > budget) goto budget_done;
            if (!append_text(context, budget + 1u, &context_used, line)) goto budget_done;
            snprintf(relation_json, sizeof(relation_json),
                "%s{\"subject_key\":\"%s\",\"predicate\":\"%s\",\"object_key\":\"%s\",\"evidence_id\":\"%s\",\"hop\":%u,\"confidence\":%.6f}",
                result_count ? "," : "", edge->subject_key, edge->predicate ? edge->predicate : "", edge->object_key,
                edge->evidence_id ? edge->evidence_id : "", current.depth + 1u, effective);
            if (!append_text(relations, sizeof(relations), &relations_used, relation_json)) { status = MEMORIA_MOBILE_INTERNAL_ERROR; goto done; }
            ++result_count;
            if (effective > best_confidence) best_confidence = effective;
            if (current.depth + 1u < max_depth && frontier_count < ACTIVATION_MAX_FRONTIER && !seen_node(visited, visited_count, next_key)) {
                snprintf(frontier[frontier_count].node_key, sizeof(frontier[frontier_count].node_key), "%s", next_key);
                frontier[frontier_count].depth = current.depth + 1u;
                frontier[frontier_count].confidence = effective;
                visited[visited_count] = frontier[frontier_count];
                ++visited_count; ++frontier_count;
            }
            if (result_count >= ACTIVATION_MAX_RESULTS) goto budget_done;
        }
    }
budget_done:
    if (!append_text(relations, sizeof(relations), &relations_used, "]")) { status = MEMORIA_MOBILE_INTERNAL_ERROR; goto done; }
    status = set_activation_response(
        response_json, result_count ? "HIT" : "UNRESOLVED", concept,
        context, relations, result_count, max_depth, best_confidence
    );
done:
    free(storage);
    free(concept); free(namespace_id); free(concept_namespace); free(json);
    return status;
}
