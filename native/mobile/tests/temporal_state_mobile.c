...
static int assert_temporal(memoria_mobile_handle *h) {
    memoria_mobile_buffer out = {0};
    CHECK(call(h,0,"{\"query\":\"what was device mode before and what is current now?\"}",&out) == MEMORIA_MOBILE_OK);
...
int main(void) {
    ...
    CHECK(call(h,1,"{\"role\":\"user\",\"text\":\"device mode is standby\",\"memory_id\":\"m1\",\"order\":10}",&out) == MEMORIA_MOBILE_OK);
    memoria_mobile_free_buffer(out); out=(memoria_mobile_buffer){0};

    CHECK(call(h,1,"{\"role\":\"assistant\",\"text\":\"device mode is broken\",\"memory_id\":\"echo\",\"order\":15,\"source_type\":\"assistant_generated\",\"source_authority\":0.35,\"ultimate_source_memory_id\":\"m1\"}",&out) == MEMORIA_MOBILE_OK);
    memoria_mobile_free_buffer(out); out=(memoria_mobile_buffer){0};

    CHECK(call(h,1,"{\"role\":\"user\",\"text\":\"device mode is active\",\"memory_id\":\"m2\",\"order\":20}",&out) == MEMORIA_MOBILE_OK);
    memoria_mobile_free_buffer(out); out=(memoria_mobile_buffer){0};

    CHECK(assert_temporal(h) == 0);
