...
int main(void) {
    ...
    /* Keep this probe below the legacy fixed episode ceiling. Its purpose is
       exclusively to prove that catalog + episode writes share one BDR handle
       and survive a full close/cold reopen. */
    for (i = 1; i <= 32; ++i) {
    ...
    CHECK(contains(out, "\"episode_id\":\"restart-32\"") || contains(out, "\"episode_ids\":[\"restart-32\"]"));
