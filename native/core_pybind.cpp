...
public:
 ...
 void register_concept(const std::string& id,const std::vector<std::string>& anchors){auto& dst=concepts_[id];for(const auto& anchor:anchors){if(std::find(dst.begin(),dst.end(),anchor)==dst.end())dst.push_back(anchor);}}
 ...
 std::vector<RankedPair> rank_concepts(const std::string& query,const ConceptMap& concepts,std::size_t top_k=2)const{py::gil_scoped_release r;return rank_map_nogil(query,concepts,nullptr,top_k);}
 std::vector<RankedPair> rank_registered(const std::string& query,const std::vector<std::string>& candidate_ids,std::size_t top_k=2)const{py::gil_scoped_release r;return rank_map_nogil(query,concepts_,candidate_ids.empty()?nullptr:&candidate_ids,top_k);}
...
private:
 ...
 int radius_;std::unordered_map<std::string,Profile>profiles_;std::unordered_map<std::string,TokenCounts>unordered_;std::unordered_map<Feature,std::uint64_t,FeatureHash>feature_df_;std::unordered_map<std::string,std::uint64_t>observations_;ConceptMap concepts_;
};
}
PYBIND11_MODULE(_core_native,m){m.doc()="Native CPU hot paths for Memoria.ia resolutive core";py::class_<ContextScorer>(m,"ContextScorer").def(py::init<int>(),py::arg("radius")=2).def("observe",&ContextScorer::observe).def("register_concept",&ContextScorer::register_concept).def("similarity",&ContextScorer::similarity).def("unordered_similarity",&ContextScorer::unordered_similarity).def("nearest",&ContextScorer::nearest,py::arg("node"),py::arg("top_k")=5).def("rank_concepts",&ContextScorer::rank_concepts,py::arg("query"),py::arg("concepts"),py::arg("top_k")=2).def("rank_registered",&ContextScorer::rank_registered,py::arg("query"),py::arg("candidate_ids"),py::arg("top_k")=2).def_property_readonly("nodes",&ContextScorer::nodes).def_property_readonly("concepts",&ContextScorer::concepts);}
