...
public:
 ...
 std::vector<std::pair<std::string,double>> nearest(const std::string& node,std::size_t top_k=5)const{std::vector<std::pair<std::string,double>> s;{py::gil_scoped_release r;if(profiles_.find(node)==profiles_.end())return s;s.reserve(profiles_.size()?profiles_.size()-1:0);for(const auto&[o,_]:profiles_){if(o!=node)s.emplace_back(o,similarity_nogil(node,o));}std::sort(s.begin(),s.end(),cmp);if(s.size()>top_k)s.resize(top_k);}return s;}
 std::vector<std::pair<std::string,double>> rank_concepts(const std::string& query,const std::unordered_map<std::string,std::vector<std::string>>& concepts,std::size_t top_k=2)const{std::vector<std::pair<std::string,double>> ranked;ranked.reserve(concepts.size());{py::gil_scoped_release r;for(const auto&[id,anchors]:concepts){double best=0.0;for(const auto&a:anchors)best=std::max(best,std::max(similarity_nogil(query,a),unordered_similarity_nogil(query,a)));ranked.emplace_back(id,best);}std::sort(ranked.begin(),ranked.end(),cmp);if(top_k&&ranked.size()>top_k)ranked.resize(top_k);}return ranked;}
 std::size_t nodes()const noexcept{return profiles_.size();}
private:
 static bool cmp(const auto&x,const auto&y){if(x.second!=y.second)return x.second>y.second;return x.first<y.first;}
