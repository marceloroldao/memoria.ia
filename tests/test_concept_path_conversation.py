...
def _build(tmp_path):
    ...
    aware = ConceptAwareConversationResolver(
        conversation,
        concepts,
        scope=_scope(),
        concept_namespace="semantic",
    ...
    return evidence, conversation, concepts, resolver
...
def _observe(evidence, conversation, subject, predicate, obj, evidence_id, source_text, *, namespace="s1"):
    ...
    conversation.provenance.register(
        evidence_id,
...
def test_third_chance_uses_unique_directed_concept_path(tmp_path):
    evidence, conversation, _concepts, resolver = _build(tmp_path)
