...
def _metrics(model, similarity):
    ...
    return top1, sum(margins) / len(margins), min(margins)


def test_resolutive_vs_tfidf_context_margin():
    ...
    r_top1, r_mean, r_min = _metrics(resolutive, resolutive.associator.similarity)
    t_top1, t_mean, t_min = _metrics(tfidf, tfidf.similarity)

    assert r_top1 == 8
    assert t_top1 == 8
    assert r_mean > t_mean
    assert r_min > t_min
