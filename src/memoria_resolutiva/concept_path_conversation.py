...
from .product_conversation import ConversationResolveResult, ConversationSemanticService, _key
...
class ConceptPathConversationResolver:
    ...
    Order is deliberately conservative:
      ...
      3. concept-relation traversal only when the query exposes exactly two
         graph anchors and exactly one directed path orientation succeeds.

    No free-form intent parser or model-generated anchor selection is used.
    """
    ...
    def __init__(
        ...
        relation_view: ConceptRelationView,
        *,
    ...
    def _anchors(self, query: str, *, namespace: str | None) -> tuple[QueryAnchor, ...]:
        ...
        # Exact lexical graph surfaces are anchors when they occur as complete
        # normalized token phrases in the query.
        ...
        # Explicit concept aliases/canonicals are recognized by n-gram lookup.
        # Only concepts that actually occur as endpoints in the current graph
        # may become anchors.
    ...
    def _path_rows_result(
        self,
        path: ConceptRelationPath,
        *,
        namespace: str | None,
        ...
        return self.conversation._result(
            "HIT",
            rows,
    ...
    def resolve_with_trace(self, *, query: str, session_id: str | None = None):
        ...
        if alias_status == "HIT":
            trace = ConceptPathConversationTrace(
                original_status=alias_trace.original_status,
                alias_status=alias_status,
                path_attempted=False,
                final_status="HIT",
                reason=alias_trace.reason or "alias_hit",
            )
        ...
        if len(anchors) != 2:
            trace = ConceptPathConversationTrace(
                ...
                alias_status=alias_status,
                path_attempted=False,
                final_status=alias_status or "UNRESOLVED",
                reason="anchor_count",
                anchors=anchors,
            )
            self.last_trace = trace
        ...
        a, b = anchors
        forward = self.relation_view.infer_path(
            a.surface,
            b.surface,
        ...
        reverse = self.relation_view.infer_path(
            b.surface,
            a.surface,
            namespace=session_id,
            context=query,
        ...
        if len(directions) != 1:
            trace = ConceptPathConversationTrace(
                ...
                reason="no_unique_direction" if directions else "no_path",
                anchors=anchors,
            )
            self.last_trace = trace
            return second, trace
        ...
        chosen = directions[0]
        # Multiple distinct best paths are intentionally not collapsed into a
        # fabricated answer. A unique top confidence path is required.
        ...
        trace = ConceptPathConversationTrace(
            ...
            reason="concept_path" if result.status == "HIT" else "missing_evidence",
            anchors=anchors,
            evidence_ids=best[0].evidence_ids if result.status == "HIT" else (),
        )
        self.last_trace = trace
        return result, trace
