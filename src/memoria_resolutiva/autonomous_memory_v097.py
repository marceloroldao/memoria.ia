...
class AutonomousTextMemoryV097:
    """Deterministic non-neural autonomous text-memory experiment.

    Semantic discovery is inspectable and intentionally not claimed O(1).
    ``exact_lookup`` is a separate resolved-address dictionary lookup.
    """
    ...
    def observe(self, text: str, *, provenance: str = 'conversation') -> ObservationDecision:
        ...
        ranked = sorted(
            ((mid, _score(terms, self._records[mid].terms)) for mid in candidate_ids),
    ...
    def exact_lookup(self, memory_id: str) -> tuple[AutonomousRecord | None, CoreMetrics]:
        ...
        metrics = CoreMetrics(0, int(record is not None), 1.0 if record else 0.0, 0.0, 1.0 if record else 0.0,
                              'same' if record else 'unresolved', True, 0.0, elapsed, 0, 0, 0, int(record is None))
    ...
    def query(self, text: str, *, top_k: int = 3) -> AutonomousQueryResult:
        ...
        candidate_ids = self._candidate_ids(terms)
        scored: list[tuple[AutonomousRecord, float]] = []
        ...
        if not scored:
            decision = 'unresolved'
            selected: list[tuple[AutonomousRecord, float]] = []
            abstained = True
        elif ambiguous and not conflict_pair:
            decision = 'unresolved'
            selected = []
            abstained = True
        else:
        ...
        hits = tuple(AutonomousHit(
            ...
            text=record.text,
            score=score,
            relation='conflict' if conflict_pair else ('same' if score >= 0.96 else 'related'),
    ...
    def load(cls, path: str | Path) -> 'AutonomousTextMemoryV097':
        ...
        for raw in payload.get('records', []):
            record = AutonomousRecord(str(raw['memory_id']), str(raw['text']), tuple(raw['terms']), int(raw['sequence']), str(raw.get('provenance','conversation')))
            obj._index(record)
