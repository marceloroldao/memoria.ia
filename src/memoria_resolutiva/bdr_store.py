...
except ImportError as exc:  # pragma: no cover - fallback environments
...
class BDRResolutiveMemory:
    """Memoria.ia persistence adapter for the direct BDR v1.1 atomic path.

    Each logical memory is submitted as its own BDR atomic sequence. The
    durability cadence may be deferred, but logical memories are never grouped
    into one larger transaction by this adapter.

    The native extension is intentionally optional: environments without the
    BDR build use the SQLite fallback selected by ``storage_backend``.
    """
