...
class EpisodicRecallService:
    """Generic ordered conversational episode index backed by EvidenceCore.

    The caller may attach optional event_type/topics when it knows them, but the
    runtime contains no domain vocabulary. Recall ranks explicit metadata and
    source-text overlap, then selects the latest justified episode. Ambiguous
    requests abstain instead of inventing an episode.
    """
    ...
    def _decode(edge: EvidenceEdge) -> Episode:
        ...
        return Episode(
            ...
            topics=tuple(t for t in topics.split(",") if t),
        )
    ...
    def episodes(self, *, namespace: str | None = None) -> tuple[Episode, ...]:
        ...
        return tuple(sorted((self._decode(e) for e in rows), key=lambda e: (e.order, e.episode_id)))
        ...
        *,
        ...
        for episode in self.episodes(namespace=namespace):
            ...
            if overlap == 0 and explicit == 0:
                continue
            score = explicit + overlap / max(1, len(qtokens))
            ranked.append((score, episode.order, episode))
        if not ranked:
